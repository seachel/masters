...
The notation $\args{\cdot}$ is used to list arguments from the conclusion that may be used in instances of terms where it occurs. We wish to show how elements of the rule conclusion propagate through a proof.
...
%Each rule of the SL can be ``instantiated'' from one of the GSL rules by defining the variables in the GSL rules. $m, n,$ and $p$ are natural numbers representing the number of non-sequent, goal-reduction sequent, and backchaining sequent premises, respectively. For $i = 1 \ldots m$, define $Q_i$. For $j = 1 \ldots n$, $s_j$ is the number of local quantifications for goal reduction sequent premise $j$; so also define $\gamma_j$, $F_j$, $x_{j,s_j}$, and $R_{j,s_j}$. For $k = 1 \ldots p$, $t_k$ is the number of local quantifications for backchaining sequent premise $k$; so also define $\gamma'_k$, $F'_k$, $y_{k,t_k}$, and $S_{k,t_k}$.
